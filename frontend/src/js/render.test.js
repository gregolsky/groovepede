...
import { tagsByFrequency, highlightMatch, timeAgo, artistInitials, renderApp } from './render.js';
...
describe('highlightMatch', () => {
  it('returns escaped text when query is empty', () => {
    expect(String(highlightMatch('Hello & World', ''))).toBe('Hello &amp; World');
    expect(String(highlightMatch('Hello', null))).toBe('Hello');
  });
  it('wraps match in mark.hl', () => {
    expect(String(highlightMatch('The Beatles', 'beat'))).toBe('The <mark class="hl">Beat</mark>les');
  });
  it('is case-insensitive', () => {
    const result = String(highlightMatch('Radiohead', 'radio'));
    expect(result).toBe('<mark class="hl">Radio</mark>head');
  });
  it('only highlights first occurrence', () => {
    const result = String(highlightMatch('ha ha ha', 'ha'));
    expect(result).toBe('<mark class="hl">ha</mark> ha ha');
  });
  it('returns escaped text when no match', () => {
    expect(String(highlightMatch('The Beatles', 'zz'))).toBe('The Beatles');
  });
  it('escapes HTML in text before wrapping', () => {
    const result = String(highlightMatch('<b>Bold</b>', 'bold'));
    expect(result).toContain('&lt;b&gt;');
    expect(result).toContain('<mark class="hl">Bold</mark>');
  });
  it('treats regex special chars in query as literals', () => {
    const result = String(highlightMatch('3.14', '3.1'));
